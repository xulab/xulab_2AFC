...
#include "/home/xulab/behavior_rig_02/Arduino/Sketchbook_rig02/xulab_2AFC/aud_2AFC_optoStim_master/settings_OneSideOptoStim_wholeTrial_cll_VIP02_150109_2.h"
// #include "Settings/ZTT/new11/settings_cued_sweep_140822.h"
...
// uint32_t lickTimeLeft[50];
// uint32_t lickTimeRight[50];
...
int define_trial_types(int leftProb, int probeTrial_frac, int preStim_cue_frac, int optoStimTrial_frac, int opto_side) {
...
while (i < maxNumTrials) {
  ...
  //if (randNumber <= optoStimTrial_frac) {
    ...
    if ((trialType[i] == LEFT and opto_side == 0) or (trialType[i] == RIGHT and opto_side == 1) ) {
...
void stimulusDelivery(int trialCount, int currentSide, char* stim_type) {
  ...
  else  // preStim_cue_on[trialCount] == 0
  {
    delay(rand_stimOnset);
    delay(BeforeTrialOptoDuration);
  ...
  if (strcmp(stim_type, "pureTone") == 0)
  {
      ...
      else // (isProbeTrial[trialCount] != 1)
      {
        toneFreq = fq_pureTone[currentSide];
...
void setup() {
 ...
 define_trial_types(leftProb, probeTrial_frac, preStim_cue_frac, optoStimTrial_frac, opto_side);
...
delay(AfterTrialOptoDuration);
digitalWrite(LaserOutPin, LOW);

delay(interTrialInterval- AfterTrialOptoDuration);
}
