...
const int rightWaterValveDuration = 18;
...
const uint16_t toneFreq[2] = {5000, 12000}; //{1000, 4000};//
const uint16_t leftFreq[3] = {6000, 7500, 9000};
const uint16_t rightFreq[3] = {12000, 14000, 16000};
const int soundAtt = 20; // 1/0.5db.
...
// uint16_t rand_tone(int side){
//   if(side == left){
//     return(leftFreq[random(0,2)]);
//   }else{
//     return(rightFreq[random(0,2)]);
//   }
// }
...
int stimulusDelivery(int sides) {
  ...
  if (sides == 0) { // left trial
    ...
    SPI_TGM.quick_tone_vol_cosramp_5ms(stimDur, leftFreq[random(0,3)], 192 - soundAtt);
   
    // tone(StimOutPin, toneFreq[0], stimDur);
    Serial.print("\nTone Freq--------------: ");
    // Serial.println(toneFreq[0]);
    Serial.println(leftFreq[random(0, 2)]);   
  }

  if (sides == 1) { // right trial
    // SPI_TGM.quick_tone_vol_cosramp_5ms(stimDur, toneFreq[1], 192 - soundAtt);
    SPI_TGM.quick_tone_vol_cosramp_5ms(stimDur, rightFreq[random(0,3)], 192 - soundAtt);
    Serial.print("\nTone Freq++++++++++++++: ");
    // Serial.println(toneFreq[1]);
    Serial.println(rightFreq[random(0, 2)]);  
  }
